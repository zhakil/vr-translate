...
public class Quest3HeadGazeManager : MonoBehaviour
{
    ...
    private void InitializeQuest3Tracking()
    {
        ...
        // Initialize smoothed values using camera transform (fallback when XR packages not available)
        smoothedHeadPosition = headCamera.transform.position;
        smoothedHeadRotation = headCamera.transform.rotation;
        previousHeadPosition = smoothedHeadPosition;
        previousHeadRotation = smoothedHeadRotation;
        
        Debug.Log("Quest3HeadGazeManager: Using camera transform for head tracking (XR packages not loaded)");
    }
    ...
    private void UpdateQuest3HeadTracking()
    {
        if (headCamera != null)
        {
            ...
            bool trackingValid = trackingConfidence >= minTrackingConfidence;
